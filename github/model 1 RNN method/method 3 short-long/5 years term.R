...
########################################################
# 1 - Get data### we select data from 2014-01-06 to 2019-07-10, 
#setting trainig data set from 2014-01-06  to 2016-10-03 (which is the 144th data point),
#setting testing data set from "2017-08-14"(which is the 189th data point) to 2019-07-10,
########################################################
########################################################
#date_1 <- which(daily_level_data$date == '2014-01-06')
#date_2 <- which(daily_level_data$date == '2016-10-03')
...
startDate4 <-  as.Date("2014-01-06") 
startDate <-  as.Date("1999-01-04") 
endDate <- as.Date("2019-07-10")

weekly_level_data<- tq_get("HSBA.L", get = "stock.prices", from = startDate4, to = endDate, complete_cases = FALSE,periodicity='weekly')
weekly_level_data<-na.omit(weekly_level_data)
daily_level_data<- tq_get("HSBA.L", get = "stock.prices", from = startDate4, to = endDate, complete_cases = FALSE)
...
########################################################
# 2 - Split Data into 3 different time interval 
########################################################
