...
########################################################
# 1 - Get data,we select data from 2004-01-05 to 2019-07-10, 
#setting trainig data set from 2004-01-05  to 2011-10-03 (which is the 405th data point),
#setting testing data set from "2017-08-14"(which is the 711th data point) to 2019-07-10,
########################################################
#date_1 <- which(daily_level_data$date == '2004-01-05')
#date_2 <- which(daily_level_data$date == '2011-10-03 ')
...
weekly_level_data<- tq_get("HSBA.L", get = "stock.prices", from = startDate2, to = endDate, complete_cases = FALSE,periodicity='weekly')
weekly_level_data<-na.omit(weekly_level_data)
daily_level_data<- tq_get("HSBA.L", get = "stock.prices", from = startDate2, to = endDate, complete_cases = FALSE)
