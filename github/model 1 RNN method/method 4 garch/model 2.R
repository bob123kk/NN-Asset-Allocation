...
specs = garchSpec(model = list(omega = 0.001, alpha = 0.2, beta = 0.3)) 
...
#prices and returns
#par(mfrow=c(2,1))       
plot(open,type="l",xlab="Time", main="method 1 open", ylab = "Price")
...
k
increase<-rep(0,k)
increase<-data.frame(increase)
decrease<-rep(0,T-k)
decrease<-data.frame(decrease)
m=1
for(i in 1:T){
  
  if(high[i]>10) increase[m,]<-high[i]
  m=m+1
}
increase
zero<-which(increase==0)
high_value<-increase[-zero,]
high_value<-na.omit(high_value)



for(i in 1:k){
  high_value[i] = open[i] + high_value[i]
}
high_value

tail(high_value)
high_value<-as.numeric(high_value)
#prices and returns
#par(mfrow=c(2,1))       
plot(high_value,type="l",xlab="Time", main="method 1 high", ylab = "Price")
#########################low price
m=1
for(i in 1:T){
  
  if(high[i]<=10) decrease[m,]<-high[i]
  m=m+1
}
decrease
zero<-which(decrease==0)

low_value<-decrease[-zero,]
low_value<-na.omit(low_value)



low_value_price<-rep(0,T-k)
for(i in 1:T-k){
  low_value_price[i] = open[i] - low_value[i]
}
low_value_price

plot(low_value_price,type="l",xlab="Time", main="method 1 low", ylab = "Price")
low_value<-low_value_price
#########################close price
open
close<-rep(0,T)
for(i in 1:T){
...
close
tail(close)
plot(close,type="l",xlab="Time", main="method 1 close", ylab = "Price")
########################################################
# 0 - form the dataset
